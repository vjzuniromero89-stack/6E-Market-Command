...
The Options 6E calendar card shows the next ordinary Monday–Friday 10:00 a.m. New York window based on CME's general FX option schedule. Its date is explicitly **not a confirmed expiry**: holidays, listed series, and the exact underlying futures month are not validated. The user's NinjaTrader 6E Option Chain currently shows the underlying futures quote but no strikes, calls, or puts. Accordingly, options quotes, open interest, Greeks, and dealer gamma are unavailable; options do not vote in the General Engine. Adding an options data source requires confirmed access and permitted app use.
