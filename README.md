...
The Options 6E research card links directly to CME's listed-expiry calendar, option quotes, open-interest profile, prior-day bulletin, CVOL and fixing methodology. It no longer computes a purported upcoming expiry: the exact listed series and its underlying futures month must be checked at CME. These links are for the user's own reference, not an automated feed. The user's NinjaTrader 6E Option Chain currently shows the underlying futures quote but no strikes, calls or puts; NinjaScript does not support options. Accordingly, options quotes, open interest, Greeks and dealer gamma remain unavailable to the dashboard, and options do not vote in the General Engine. Adding an options source requires verified coverage and permitted app use. See `OPCIONES-CME.md` for the research, interpretation limits and integration roadmap.
