...
export function buildDailyBrief(general) {
  const items = [];
  ...
  if (general.fxReady) {
    ...
    items.push({ label: 'EUR y USD', text: 'Faltan cestas FX completas, sincronizadas y recientes; no se asigna dirección.', tone: 'unavailable' });
  }
