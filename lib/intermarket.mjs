...
export function presentIntermarket(snapshot, root, now = Date.now()) {
  if (!snapshot || snapshot.root !== root || snapshot.source !== 'ninjatrader_futures_chart' ||
    ...
    return { root, status: 'unavailable', price: null, open: null, barTimeUtc: null, instrument: null, asOf: null };
  }
  ...
  return {
    ...
    open: live && Number.isFinite(snapshot.open) ? snapshot.open : null,
    barTimeUtc: live ? snapshot.barTimeUtc : null,
