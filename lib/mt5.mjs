import { redis } from './legacy-redis.mjs';
import { SYMBOLS } from './market-context.mjs';
import { PERIODS, calculateStrengths } from './timeframes.mjs';

export const MT5_MAX_AGE = 10000;
export const mt5Key = () => `6emc:${process.env.FEED_NAMESPACE || 'production'}:mt5:fx:v1`;
...
export function validateMT5(body, now = Date.now()) {
  ...
  if (!Array.isArray(body.quotes) || body.quotes.length !== 8) throw Error('Eight symbols required');
  const seen = new Set(), reference = targets(sent);
  const quotes = body.quotes.map(q => {
    if (!q || !SYMBOLS.includes(q.symbol) || seen.has(q.symbol)) throw Error('Invalid symbol');
    seen.add(q.symbol);
    if (q.bid === null && q.ask === null) return { symbol: q.symbol, bid: null, ask: null, tickAt: null, baselines: {} };
    if (!priceOK(q.bid) || !priceOK(q.ask) || q.ask < q.bid || !Number.isSafeInteger(q.tickAt) || q.tickAt <= 0 || q.tickAt > sent + 2000) throw Error('Invalid tick');
    const baselines = {};
  ...
  return { schemaVersion: 1, mode: body.mode, connected: body.connected, sentAt: sent, receivedAt: now, quotes };
}
...
export function presentMT5(snapshot, now = Date.now(), reason = null) {
  ...
  const quotes = SYMBOLS.map(symbol => {
    ...
    return { symbol, price: q?.bid ?? null, bid: q?.bid ?? null, ask: q?.ask ?? null,
      // Cohort sampling time, distinct from last quote time; both are shown.
...
export async function mt5Context() {
  try {
    const stored = await redis(['GET', mt5Key()]);
    return presentMT5(stored ? JSON.parse(stored) : null);
  } catch { return presentMT5(null, Date.now(), 'storage_unavailable'); }
}
export const STORE_MT5 = `local old=redis.call('GET',KEYS[1]); if old then local v=cjson.decode(old); if v.sentAt >= tonumber(ARGV[2]) then return 0 end; if tonumber(ARGV[3])-v.receivedAt < 750 then return 2 end end; redis.call('SET',KEYS[1],ARGV[1],'EX',120); return 1`;
