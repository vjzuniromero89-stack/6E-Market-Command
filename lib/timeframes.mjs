...
export function calculateStrengths(quotes, period = '1h') {
  const componentsFor = entries => entries.map(([symbol, inverse]) => {
  ...
  const aligned = components => new Set(components.map(c => c.asOf)).size === 1 && new Set(components.map(c => c.reference)).size === 1;
  const calculate = entries => {
    const components = componentsFor(entries);
    if (components.some(c => !c) || !aligned(components)) return null;
  ...
  const partialMean = entries => {
    const components = componentsFor(entries).filter(Boolean);
    if (components.length < 2 || components.length === entries.length || !aligned(components)) return null;
    return { meanChange: components.reduce((sum, item) => sum + item.change, 0) / components.length,
      count: components.length, total: entries.length, asOf: components[0].asOf, reference: components[0].reference, period };
  };
  ...
  return { USD: calculate(usd), EUR: calculate(eur), USD_EX_EUR: calculate(usd.filter(([symbol]) => symbol !== 'EUR/USD')),
    EUR_EX_USD: expanded ? calculate(eur.filter(([symbol]) => symbol !== 'EUR/USD')) : null,
    USD_PARTIAL: partialMean(usd), EUR_PARTIAL: partialMean(eur) };
}
